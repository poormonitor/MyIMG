...
const submitForm = () => {
    ...
    axios.post("/login", {
        ...
        .then((response) => {
            sessionStorage.setItem("access_token_myimg", response.data.access_token)
            sessionStorage.setItem("email_myimg", loginForm.email)
            if (rememberMe.value) {
                localStorage.setItem("access_token_myimg", response.data.access_token)
            }
            setTimeout(() => {
                loadingLogin.value = false
...
<template>
    <div class="mx-auto w-4/5 md:w-2/3 lg:w-1/2 mt-20 py-12 lg:py-16 border-2 rounded-3xl">
        ...
        <el-form :model="loginForm" :rules="rules" label-position="left" label-width="33%" size="large"
            class="mx-10 md:mx-24 lg:mx-36">
            <el-form-item label="Email">
                <el-input placeholder="Email" autocomplete="email" v-model="loginForm.email" />
            </el-form-item>
            ...
            <p class="text-sm text-center">
                No account? 
                <router-link class="text-blue-600 pl-3" :to="{name:'register'}">Register</router-link>
            </p>
            <div class="flex justify-center pt-2 lg:pt-4">
                <el-button class="lg:w-2/3 w-full" type="primary" @click="submitForm"
                    :loading="loadingLogin">Login</el-button>
            </div>
        </el-form>
    </div>
</template>