...
const SearchPic = () => {
    showImages.value = myImages.value.filter(item => item.name.includes(searchInput.value))
}
...
<template>
    <div class="ml-8 lg:ml-16 mt-8 lg:mt-12">
        ...
        <div class="flex justify-center mt-3">
            <el-input v-model="searchInput" size="large" class="w-64 lg:w-96" placeholder="Search" :prefix-icon="Search"
                @change="SearchPic" />
        </div>
        <div class="flex mt-4 items-center justify-center">
            <span class="text-sm text-sky-600 mr-2">Mode of Copy</span>
            <el-radio-group v-model="copyProp" size="small">
                <el-radio-button v-for="i in copyOptions.length" :key="i - 1" :label="i - 1">
                    {{ copyOptions[i - 1] }}
                </el-radio-button>
            </el-radio-group>
        </div>
    </div>
    <p class="text-center text-zinc-400 text-xl md:text-3xl mt-12" v-if="!showImages.length">No images found.</p>
    <div class="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-2 mx-6 md:mx-12 lg:mx-16 mt-6 md:mt-8 lg:mt-10"
        v-else>
        <div v-for="item in showImages"
            ...
            <el-popover placement="top-start" title="Copied!" :width="200" trigger="click"
                content="The URL has been copied to the clipboard." :hide-after="50">
                <template #reference>
                    <div class="flex-1 m-0.5 md:m-2 p-3 break-all" @click="copyToClipboard(getURL(item.url, item.pid))">
                        <p class="text-xs text-indigo-900">{{ item.pid }}</p>
                        <p class="text-xs font-bold text-blue-800">{{ item.indate }}</p>
                        <p class="text-lg mt-1 font-bold" v-html="highLightName(item.name)"></p>
                        <p class="text-xs font-semibold underline decoration-indigo-500/30">{{ item.url }}</p>
                    </div>
                </template>
            </el-popover>
            <div class="absolute right-3 top-3 delete-button transition-all">
                ...
                <NameSetter class="ml-1" :pid="item.pid" :origin="item.name" @finish="setNewName" />
            </div>
        </div>
    </div>
</template>
